	...
	var imgdefw;  //图片默认宽度
	var imgdefh;  //图片默认高度
	var offsetx =202; //图片位置位移x
	var offsety = 80; //图片位置位移y
	var divx = 540; //外框宽度
	var divy = 266; //外框高度
	var cutx = 150;  //裁减宽度
	var cuty = 112;  //裁减高度
...
//
function changeimage(selectimage){
	cut_img = document.getElementById('cut_img');
	cut_img.src=selectimage.src;
}	
function radiochangeimage(imagekey){
	cut_img = document.getElementById('cut_img');
	var imagekeystr="pic_"+imagekey;
	cut_img.src=document.getElementById(imagekeystr).src;
	var image_width_keystr="picwidth_"+imagekey;
	var image_height_keystr="picheight_"+imagekey;
	var select_image_width=document.getElementById(image_width_keystr).value;
	var select_image_height=document.getElementById(image_height_keystr).value;
	cut_img.width=select_image_width;
	cut_img.height=select_image_height;
	avatarinit();
...
function getPosition(e){  
	...
	var l = $("#img_track").css('left');
	var t = $("#img_track").css('top');
...
function gripinit(){
	...
	var left = (grip_minx + (grip_pos / 10 * (grip_maxx - grip_minx))) + "px";
	img_grip.style.left = left;
	var top = grip_y + "px";
	img_grip.style.top = top
